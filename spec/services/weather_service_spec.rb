  ...
  describe "#hourly_weather" do
    it "should return a response body parsed from JSON" do
      VCR.use_cassette("forecast") do
        ...
        expect(parsed_json).to be_an Array
        expect(parsed_json.first[:time]).to be_a String
        expect(parsed_json.first[:temp_f]).to be_a Float
        expect(parsed_json.first[:condition][:text]).to be_a String
        expect(parsed_json.first[:condition][:icon]).to be_a String
      end
    end
  end
end